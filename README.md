...
| 機能             | 概要                                                                                           |
| ---------------- | ---------------------------------------------------------------------------------------------- |
| セーブ＆ロード   | 現在の `ScenarioState`（シーン ID・step インデックス）をファイルに永続化し、再開できるようにする |
| システムボタン   | 画面上部などに Save / Load / Prof（プロフィール）/ Conf（設定）ボタンを配置する                 |
| シナリオ選択肢   | `Step` に `Choice { options: Vec<ChoiceOption> }` バリアントを追加し、分岐シナリオを実現する   |
| 立ち絵表示・操作 | キャラクター画像を画面中央付近に表示し、位置・表情・フリップなどを `SceneCommand` で制御する   |
| 背景切り替え     | `Command(ChangeBackground(...))` を実際に処理し、`background.rs` の背景画像を差し替える        |
